...
class PositDivSqrtSpec extends ChiselFlatSpec {
  ...
  private class PositDivSqrtTest(
      c: PositDivSqrt,
      num1: Int,
      num2: Int,
      sqrtOp: Boolean,
    ...
    while (
      if (sqrtOp) peek(c.io.validOut_sqrt) == BigInt(0)
      else peek(c.io.validOut_div) == BigInt(0)
    ) {
      step(1)
    }
...
//    chisel3.testers.Driver.execute(Array("--generate-vcd-output", "on", "--target-dir", "test_run_dir/div_sqrt", "--top-name", "dsvcd"), () => new PositDivSqrt(totalBits, es)) {
//      c => new PositDivSqrtTest(c, num1, num2, sqrtOp, expect)
//    }

    iotesters.Driver(() => new PositDivSqrt(nbits, es)) { c =>
      new PositDivSqrtTest(c, num1, num2, sqrtOp, expect)
    }
  }
