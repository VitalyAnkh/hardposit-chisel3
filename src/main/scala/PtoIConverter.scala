...
class PtoIConverterCore(val nbits: Int, val es: Int, val intWidth: Int)
    extends Module
  ...
  val inRange =
    Mux(
      io.unsignedOut,
      num.exponent < intWidth.S,
      num.exponent < (intWidth - 1).S
    )
  val specialCase = ~inRange | num.isNaR | (num.sign & io.unsignedOut)
  val specialCaseOut =
    Mux(
      specialCase & io.unsignedOut & (~num.sign | num.isNaR),
      maxUnsignedInteger(intWidth).U,
      0.U
    ) |
      Mux(
        specialCase & ~io.unsignedOut,
        maxSignedInteger(intWidth).U + (~num.isNaR & num.sign),
        0.U
      )
...
class PtoIConverter(val nbits: Int, val es: Int, val intWidth: Int)
    extends Module
    with HasHardPositParams {
  val io = IO(new Bundle {
    ...
    val unsignedOut = Input(Bool())
    val roundingMode =
      Input(Bool()) // Indicate rounding mode 0 for round to nearest even(RNE)
