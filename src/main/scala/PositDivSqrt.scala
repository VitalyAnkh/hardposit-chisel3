...
class PositDivSqrtCore(val nbits: Int, val es: Int)
    extends Module
  ...
  val isNaR =
    Mux(io.sqrtOp, num1.sign | num1.isNaR, num1.isNaR | num2.isNaR | divZ)
  ...
  val radicand = Mux(
    io.sqrtOp && num1.exponent(0).asBool(),
    num1.fraction << 1,
    num1.fraction
  )
  ...
  val rem = Mux(
    readyIn && io.sqrtOp,
    radicand(maxFractionBitsWithHiddenBit, maxFractionBitsWithHiddenBit - 1),
    0.U
  ) |
    Mux(readyIn && !io.sqrtOp, radicand.asUInt(), 0.U) |
    Mux(
      !readyIn && sqrtOp_stored,
      remHi << 2.U | remLo >> maxFractionBits.U,
      0.U
    ) |
  ...
  result.fraction := frac_out(
    maxDividerFractionBits - 1,
    maxDividerFractionBits - maxFractionBitsWithHiddenBit
  )
  ...
  io.trailingBits := frac_out(
    maxDividerFractionBits - maxFractionBitsWithHiddenBit - 1,
    trailingBitCount
  )
...
class PositDivSqrt(val nbits: Int, val es: Int)
    extends Module
    with HasHardPositParams {
  val io = IO(new Bundle {
    val validIn = Input(Bool())
