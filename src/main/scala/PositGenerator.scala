...
class PositGenerator(val nbits: Int, val es: Int)
    extends Module
  ...
  val regime =
    Mux(
      negExp,
      -io.in.exponent(maxExponentBits - 1, es),
      io.in.exponent(maxExponentBits - 1, es)
    ).asUInt()
  ...
  val expFrac =
    if (es > 0)
      Cat(Mux(negExp, 1.U(2.W), 2.U(2.W)), exponent, fraction, io.trailingBits)
        .asSInt()
    else
  ...
  val uT_uS_posit =
    (expFrac >> offset)(nbits - 2 + trailingBitCount, 0).asUInt()
