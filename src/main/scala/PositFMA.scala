...
class PositFMACore(val nbits: Int, val es: Int)
    extends Module
  ...
  val normProductFraction =
    Mux(
      prodOverflow,
      productFraction(maxMultiplierFractionBits - 1, 1),
      productFraction(maxMultiplierFractionBits - 2, 0)
    )
  ...
  val addendFraction =
    Mux(addendIsZero, 0.U, Cat(num3.fraction, 0.U(maxFractionBits.W)))
  ...
  val adjFmaFraction =
    Mux(
      fmaOverflow,
      fmaFraction,
      Cat(fmaFraction(maxMultiplierFractionBits - 2, 0), 0.U(1.W))
    )
  ...
  val normFmaFraction =
    (adjFmaFraction << normalizationFactor)(maxMultiplierFractionBits - 1, 0)
  ...
  result.fraction := normFmaFraction(
    maxMultiplierFractionBits - 1,
    maxMultiplierFractionBits - maxFractionBitsWithHiddenBit
  ).asUInt()

  io.trailingBits := normFmaFraction(
    maxFractionBitsWithHiddenBit - 1,
    maxFractionBitsWithHiddenBit - trailingBitCount
  )
  io.stickyBit := prodStickyBit | lFracStickyBit | normFmaFraction(
...
class PositFMA(val nbits: Int, val es: Int)
    extends Module
    with HasHardPositParams {
