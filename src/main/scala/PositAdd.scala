...
class PositAddCore(val nbits: Int, val es: Int)
    extends Module
  ...
  val largeFrac =
    Cat(
      Mux(num1magGt, num1.fraction, num2.fraction),
      0.U((maxAdderFractionBits - maxFractionBitsWithHiddenBit - 1).W)
    )
  ...
  val smallFrac =
    Cat(
      Mux(num1magGt, num2.fraction, num1.fraction),
      0.U((maxAdderFractionBits - maxFractionBitsWithHiddenBit - 1).W)
    )
  ...
  val smallFracStickyBit =
    Mux(
      expDiff > (maxAdderFractionBits - maxFractionBitsWithHiddenBit - 1).U,
      (smallFrac & ((1.U << (expDiff - (maxAdderFractionBits - maxFractionBitsWithHiddenBit - 1).U)) - 1.U))
        .orR(),
      false.B
    )
  ...
  val adjAdderFrac =
    Mux(
      sumOverflow,
      adderFrac(maxAdderFractionBits - 1, 1),
      adderFrac(maxAdderFractionBits - 2, 0)
    )
  ...
  result.fraction := normFraction(
    maxAdderFractionBits - 2,
    maxAdderFractionBits - maxFractionBitsWithHiddenBit - 1
  )

  io.trailingBits := normFraction(
    maxAdderFractionBits - maxFractionBitsWithHiddenBit - 2,
    maxAdderFractionBits - maxFractionBitsWithHiddenBit - trailingBitCount - 1
  )
...
class PositAdd(val nbits: Int, val es: Int)
    extends Module
    with HasHardPositParams {
  require(nbits > es)
  require(es >= 0)
