...
class ItoPConverterCore(val nbits: Int, val es: Int, intWidth: Int)
    extends Module
  ...
  result.fraction := {
    if (narrowConv)
      ...
      Cat(
        shiftedIntVal(intWidth - 1, 0),
        0.U((maxFractionBitsWithHiddenBit - intWidth).W)
      )
  }

  io.trailingBits := {
    if (narrowConv)
      Cat(shiftedIntVal, 0.U(trailingBitCount.W))(
        intWidth - maxFractionBitsWithHiddenBit + trailingBitCount - 1,
        intWidth - maxFractionBitsWithHiddenBit
      )
    else 0.U
  }
  ...
  io.stickyBit := {
    if (narrowConv)
      Cat(shiftedIntVal, 0.U(trailingBitCount.W))(
        intWidth - maxFractionBitsWithHiddenBit - 1,
        0
      ).orR()
    else false.B
  }
...
class ItoPConverter(val nbits: Int, val es: Int, intWidth: Int)
    extends Module
    with HasHardPositParams {
  val io = IO(new Bundle {
    val integer = Input(UInt(intWidth.W))
