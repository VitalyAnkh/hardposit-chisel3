...
class UnpackedPosit(val nbits: Int, val es: Int)
    extends Bundle
  ...
  val fraction = UInt(
    maxFractionBitsWithHiddenBit.W
  ) // TODO Transfer only fraction bits without hidden bit
  val isZero = Bool()
  val isNaR = Bool()
...
trait HasHardPositParams {
  ...
  require(
    nbits > es + 3,
    s"Posit size $nbits is inadequate for exponent size $es"
  )
  ...
  def maxFractionBitsWithHiddenBit: Int =
    getMaxFractionBitsWithHiddenBit(nbits, es)

  def getMaxExponentBits(n: Int, e: Int): Int = log2Ceil(n) + e + 2

  def getMaxFractionBits(n: Int, e: Int): Int = if (e + 3 >= n) 1 else n - 3 - e

  def getMaxFractionBitsWithHiddenBit(n: Int, e: Int): Int =
    getMaxFractionBits(n, e) + 1

  def maxAdderFractionBits: Int =
    maxFractionBitsWithHiddenBit + trailingBitCount + stickyBitCount + 1

  def maxMultiplierFractionBits: Int = 2 * maxFractionBitsWithHiddenBit

  def maxDividerFractionBits: Int =
    maxFractionBitsWithHiddenBit + trailingBitCount + stickyBitCount + 1
